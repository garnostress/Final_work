...
class MainPage(APIView):
    def first_page(request):
        return HttpResponse("API Service for ordering goods for retail chains")
            ...
            try:
                ...
                if user_serializer.is_valid():
                    ...
                    return Response({'status': False, 'error': user_serializer.errors},
                                    status=status.HTTP_403_FORBIDDEN)
        return Response({'status': False, 'error': 'Required fields not specified'},
    ...
    def post(self, request, *args, **kwargs):
        ...
        if {'email', 'token'}.issubset(request.data):
            ...
            if token:
                ...
                return Response({'Status': False, 'Errors': 'Incorrect token or email specified'})

        return Response({'Status': False, 'Errors': 'All required arguments are not provided'})
...
class LoginUser(APIView):
    ...
    def post(self, request, *args, **kwargs):
        if {'email', 'password'}.issubset(request.data):
            ...
            return Response({'status': False, 'error': 'Failed to login'}, status=status.HTTP_403_FORBIDDEN)
...
class UserDetails(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response({'Status': False, 'Errors': 'All required arguments are not specified (Password)'})
        ...
        if {'items'}.issubset(request.data):
            for item in request.data["items"].split(','):
                try:
                    ...
                    return Response({'status': False, 'error': 'Invalid field type (items).'},
                                    status=status.HTTP_400_BAD_REQUEST)

            return Response({'status': True}, status=status.HTTP_204_NO_CONTENT)

        return Response({'status': False, 'error': 'Contact IDs not specified'},
                        status=status.HTTP_400_BAD_REQUEST)

class ProviderUpdate(APIView):
    '''
    Update or add a store price list (if there is no store, it will be created)
    '''
    permission_classes = [IsAuthenticated]
        ...
        if url:
            ...
            try:
                ...
                if shop.name != data['shop']:
                    return Response({'status': False, 'error': 'The file contains an incorrect store name!'},
                                    status=status.HTTP_400_BAD_REQUEST)
...
class ProviderState(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response({'status': False, 'error': 'The "Status" field is not specified.'}, status=status.HTTP_400_BAD_REQUEST)
...
class ProviderOrders(APIView):
    ...
    def get(self, request, *args, **kwargs):
        '''
        Receipt of completed orders by suppliers
        '''
        if request.user.type != 'shop':
            return Response({'status': False, 'error': 'Only for shops'}, status=status.HTTP_403_FORBIDDEN)
...
class ProductView(APIView):
    '''
    Search for all or specific products
    '''
    def get(self, request, *args, **kwargs):
...
class CartView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if items:
            try:
                ...
                Response({'Status': False, 'Errors': 'Invalid request format'})
            else:
    ...
    def put(self, request, *args, **kwargs):
        '''
        Changing the quantity of a specific product in the cart
        '''
        items = request.data.get('items')
        if items:
            try:
        ...
        if {'id', 'contact'}.issubset(request.data):
            if request.data['id'].isdigit():
                try:
                    ...
                    print(error)
                    return Response({'Status': False, 'Errors': 'Incorrect arguments'})
                else:
                    if is_updated:
                        return Response({'Status': True})
